...
app.post('/item_descarte', async (req, res) => {
    ...
    try {
        ...
        const insertItemQuery = `INSERT INTO item_descarte (nome, descricao, riscos, empresa_id) VALUES (:nome, :descricao, :riscos, 1) RETURNING id INTO :id`;
        const itemResult = await connection.execute(insertItemQuery, { nome, descricao, riscos, id: { type: oracledb.NUMBER, dir: oracledb.BIND_OUT } });

        const itemId = itemResult.outBinds.id[0];

        await connection.execute(`COMMIT`);

        res.status(201).json({ message: 'Item de descarte cadastrado com sucesso!', id: itemId });
